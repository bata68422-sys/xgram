...
function renderShopGifts() {
    ...
    container.innerHTML = shopData.gifts.map(gift => {
        ...
        return `
            <div class="shop-item ${gift.limited ? 'limited' : ''} ${isSoldOut ? 'sold-out' : ''} ${rarityClass}" onclick="buyGift('${gift.id}')">
...
function renderShopNFTs() {
    ...
    container.innerHTML = shopData.nfts.map(nft => {
        ...
        const imageHtml = isImage 
            ? `<img src="${nft.image}" class="shop-nft-img" alt="${nft.name}">`
...
function addShopNft() {
    ...
    socket.emit('admin_add_shop_nft', { name, image, price, quantity, rarity, upgradeable });
...
renderMyItems = function() {
    ...
    if (myNfts.length === 0) {
        ...
        nftsContainer.innerHTML = myNfts.map((n, i) => {
            ...
            const imageHtml = isImage 
                ? `<img src="${n.image}" class="nft-thumb">`
                : `<span class="nft-emoji">${n.image}</span>`;
            return `
...
// ===== ГРУППЫ И КАНАЛЫ - ПОИСК И ВСТУПЛЕНИЕ =====
function openBrowseGroups() {
    socket.emit('get_all_groups');
    socket.emit('get_all_channels');
    openModal('browse-groups-modal');
}

socket.on('all_groups', (groups) => {
    const container = document.getElementById('browse-groups-list');
    if (groups.length === 0) {
        container.innerHTML = '<p style="color:var(--text-muted)">Нет групп</p>';
        return;
    }
    container.innerHTML = groups.map(g => `
        <div class="browse-item">
            <div class="browse-avatar">👥</div>
            <div class="browse-info">
                <div class="browse-name">${g.name}</div>
                <div class="browse-meta">${g.membersCount} участников</div>
            </div>
            ${g.isMember 
                ? '<button class="btn-joined" disabled>Вы участник</button>'
                : `<button class="btn-join" onclick="joinGroup('${g.id}')">Вступить</button>`
            }
        </div>
    `).join('');
});

socket.on('all_channels', (channels) => {
    const container = document.getElementById('browse-channels-list');
    if (channels.length === 0) {
        container.innerHTML = '<p style="color:var(--text-muted)">Нет каналов</p>';
        return;
    }
    container.innerHTML = channels.map(c => `
        <div class="browse-item">
            ...
            <div class="browse-info">
                ...
                <div class="browse-meta">${c.subscribersCount} подписчиков</div>
            </div>
            ${c.isSubscribed 
                ? '<button class="btn-joined" disabled>Подписан</button>'
                : `<button class="btn-join" onclick="joinChannel('${c.id}')">Подписаться</button>`
            }
        </div>
    `).join('');
});
...
function joinGroup(groupId) {
    ...
    closeModal('browse-groups-modal');
}
...
renderMyItems = function() {
    ...
    if (myNfts.length === 0) {
        ...
        nftsContainer.innerHTML = myNfts.map((n, i) => {
            const level = n.level || 1;
            const canUpgrade = level < (n.maxLevel || 5);
            const upgradeCost = level * 100;
            const glowClass = level >= 3 ? 'nft-glow-purple' : level >= 2 ? 'nft-glow-blue' : '';
            ...
            return `
                <div class="my-item nft-item-animated rarity-${n.rarity} rarity-card-${n.rarity} ${glowClass} ${isSelected ? 'selected' : ''}" 
                     style="animation-delay: ${i * 0.05}s"
                     data-tooltip="${n.name}&#10;Редкость: ${n.rarity}&#10;Уровень: ${level}&#10;Код: #${code}&#10;Цена продажи: ${sellPrice} 🐱&#10;Комиссия: ${commission} 🐱">
...
function switchCreateType(type) {
    ...
    document.querySelectorAll('.type-tab').forEach(t => {
        t.classList.toggle('active', t.textContent.includes(type === 'group' ? 'Группа' : 'Канал'));
    });
}
