...
if (!db.shop) {
    db.shop = {
        ...
        nfts: [
            ...
            { id: 'nft_phoenix', name: 'Феникс', price: 500, image: '🔥', rarity: 'rare', sellPrice: 250 },
            { id: 'nft_robot', name: 'Робот', price: 300, image: '🤖', rarity: 'uncommon', sellPrice: 150 }
        ],
        limitedOwners: {},
...
io.on('connection', (socket) => {
    ...
    socket.on('buy_gift', (giftId) => {
        ...
        if (!user.gifts) user.gifts = [];
        user.gifts.push({ ...gift, date: Date.now() });
        saveDB();
    ...
    socket.on('buy_nft', (nftId) => {
        ...
        if (!user.nfts) user.nfts = [];
        user.nfts.push({ ...nft, date: Date.now(), tokenId: Date.now().toString(36) });
        saveDB();
    ...
    socket.on('create_custom_gift', ({ name, emoji, price }) => {
        if (!currentUser || !isAdmin(currentUser)) return;
    ...
    socket.on('upgrade_nft', (nftIndex) => {
        ...
        const maxLevel = nft.maxLevel || 5;
        if (currentLevel >= maxLevel) return socket.emit('shop_error', 'Максимальный уровень');
        
        const upgradeCost = currentLevel * 100; // 100, 200, 300...
        if (user.coins < upgradeCost) return socket.emit('shop_error', `Нужно ${upgradeCost} котиков`);
        
        user.coins -= upgradeCost;
        nft.level = currentLevel + 1;
        nft.sellPrice = Math.floor((nft.sellPrice || nft.price / 2) * 1.5);
        nft.upgradeable = true; // Все NFT можно улучшать
        saveDB();
        
        socket.emit('nft_upgraded', { nft, newBalance: user.coins, nfts: user.nfts });
    });
    ...
    socket.on('send_group_message', ({ groupId, text, type, media }) => {
        ...
        io.to(groupId).emit('group_message', { groupId, message });
    });
    ...
    // Отправка сообщения
    socket.on('send_message', ({ to, text, type, media }) => {
        if (!currentUser || !db.users[to]) return;
    ...
    // Обновление профиля
    socket.on('update_profile', ({ displayName, avatar, status, theme, telegram }) => {
        if (!currentUser) return;
    ...
    socket.on('admin_add_shop_gift', ({ name, emoji, price, image }) => {
        if (!currentUser || !isAdmin(currentUser)) return;
